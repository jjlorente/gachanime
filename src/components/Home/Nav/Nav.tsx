    ...
    <div className={classNav ? "Nav Nav-active" : "Nav "}>
      ...
      <div>
        <div className={`toggle-section link-reset`}  onClick={onToggleMenu} >
          {classNav ? props?.userData.username : ""}{svgMenu}
        </div>
          ...
          {navItems.map((item, index) => {
            return (
              <Link 
                to={item.link} 
                key={index} 
                className={`nav-section link-reset ${item.link === props.activeIndex ? 'nav-active' : ''}`} 
                onClick={() => props.setActiveIndex(item.link)}
              >
                {icons[item.link as NavItemLink]}
                {classNav ? <span className={props.activeIndex === item.link ? "span-section-nav-active color-active" : "span-section-nav-active color-inactive"}>{item.label}</span> : <span className='span-section-nav-inactive'>{item.label}</span> }
              </Link>
            );
          })}
        </div>
