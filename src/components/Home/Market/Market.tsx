...
export const Market = () => {
  ...
  dataMarket !== undefined 
    ? currentCards = dataMarketSelected.slice(firstPostIndex, lastPostIndex) 
    : null;
  ...
  return (
    <div className='Market'>
      <MarketFilter 
        setDataMarketSelected={setDataMarketSelected} 
            ...
            currentCards.map((card: any, index: number) => {
              ...
              return (
                <div key={`card-${index}`} style={{ display: "flex", flexDirection: "column", gap: ".3rem" }}>
                  <div
                    className={
                      'container-card-obtained container-card card-market' + ' border-collection' + rarityClass
                    }
                  >
                    <img
                  ...
                  <div className='cnt-info-price'>
                    <span className='span-username-market'>
                      {t('market.owner')} {card['user'].username}
                    </span>
                    ...
                    {userId === card['user']._id ? (
                      <button 
                        key={`cancel-btn-${index}`} 
                        className="cancel-btn-market jaro-regular" 
                        onClick={() => {setOpenModal(true); setIdCard(card['card']._id); setPrice(card['price']); setMode(false)}}
                      >
                        {t('summon.cancel')}
                      </button>
                    ) : (
                      <button 
                        key={`buy-btn-${index}`} 
                        className="confirm-btn-market jaro-regular" 
                        onClick={() => {setOpenModal(true); setIdCard(card['card']._id); setPrice(card['price']); setUserCardId(card['user']._id); setMode(true)}}
                      >
                        {t('market.buyButton')}
                      </button>
                    )}
